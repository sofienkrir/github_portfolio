...
function ProjectCard({ project, featured, onClick }) {
  ...
  return (
    ...
    >
      <p className="project-type" style={{ color: c.accent }}>{project.type}</p>
      ...
      <div className="project-techs">
        ...
        {project.tech.length > (featured ? 8 : 5) && (
          <span className="tag" style={{ color: "var(--text-3)", background: "transparent" }}>
            +{project.tech.length - (featured ? 8 : 5)}
          </span>
        )}
      </div>
  ...
  return (
    <section id="projects" className="section">
      <div className="container">
        ...
        <h2 className="section-title" style={{ marginBottom: 8 }}>Projets réalisés</h2>
        ...
        <div className="projects-featured">
          {featured.map((p) => (
            <ProjectCard key={p.id} project={p} featured onClick={() => setActive(p)} />
          ))}
        </div>

        {/* Secondary */}
        <div style={{ marginBottom: 16 }}>
          <p style={{ fontSize: 13, color: "var(--text-3)", fontFamily: "var(--font-mono)", marginBottom: 16 }}>
            — autres projets
          </p>
        </div>
